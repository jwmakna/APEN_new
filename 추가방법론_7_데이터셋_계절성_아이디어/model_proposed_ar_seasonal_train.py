...
# [계절 매칭 학습구간] 학습구간을 테스트구간과 독립적으로 정하지 않고,
# "테스트 마지막날의 정확히 YEAR_OFFSET_DAYS일 전"을 학습 마지막날로 삼아
# 자동으로 계산한다 - 이렇게 하면 학습구간이 항상 테스트구간과 같은
# 달력상 계절(월/일)에 오게 되고, 테스트기간의 실제 가격은 전혀 들여다보지
# 않으므로(달력 규칙만 사용) data leakage가 아니다.
TRAIN_LENGTH_DAYS = int(os.environ.get("TRAIN_LENGTH_DAYS", "90"))    # 학습 기간 길이(일)
YEAR_OFFSET_DAYS = int(os.environ.get("YEAR_OFFSET_DAYS", "365"))     # 테스트보다 몇 일 전의 "작년"을 쓸지

TRAIN_END = TEST_END - pd.Timedelta(days=YEAR_OFFSET_DAYS)            # 학습 마지막일 = 테스트 마지막일의 1년 전
TRAIN_START = TRAIN_END - pd.Timedelta(days=TRAIN_LENGTH_DAYS - 1)    # 거기서부터 90일을 거슬러 올라간 날
...
print()
print(f"=== 논문 제안 모형 AR [계절 매칭 학습구간: 테스트 {YEAR_OFFSET_DAYS}일 전부터 {TRAIN_LENGTH_DAYS}일] - z03 블록18 결과 ===")
