...
# 가격 신호로 학습하게 하려는 목적. 평가(7번 이후)는 원래 가격 그대로 씀.
...
# =====================================================================
# 2-B. [계절 정규화] 학습기간 각 날짜의 "최근 30일 평균 DA가격" 기준선을
#      날짜별 딕셔너리로 미리 계산해둔다.
...
train_baseline_by_date = {}                                    # 날짜 -> 기준선(최근 30일 평균 DA가격) 딕셔너리
for this_date in train_dates_sorted:                              # 학습 날짜를 하나씩 순서대로
    ...
    train_baseline_by_date[this_date] = daily_avg_da_all.loc[recent_past_dates].mean()   # 그 30일 평균 = 기준선
...
train_price_baseline = np.zeros(n_train_obs)       # [계절 정규화] 이 행 날짜의 기준선 값을 담을 빈 배열
...
for _, one_row in train_rows.iterrows():               # train_rows 를 한 줄씩 순서대로 확인
    ...
    train_price_baseline[row_counter] = train_baseline_by_date[one_row["local_date"]]  # [계절 정규화] 이 날짜의 기준선
    row_counter = row_counter + 1                                # 카운터를 하나 증가시킴

train_da_price_norm = (train_da_price / train_price_baseline) * train_da_price.mean()   # [계절 정규화] 정규화 후
                                                                                          # 스케일을 원래 평균 수준으로 복원 (W1/W2 균형 유지)
train_rt_price_norm = (train_rt_price / train_price_baseline) * train_da_price.mean()    # RT도 같은 배율로 복원

print()
print("[계절 정규화] 학습기간 기준선(최근 30일 평균 DA가격) 범위:",
      f"{train_price_baseline.min():.2f} ~ {train_price_baseline.max():.2f}")
print("[계절 정규화] 정규화된 DA가격 범위:",
      f"{train_da_price_norm.min():.4f} ~ {train_da_price_norm.max():.4f}",
      f"(참고: 학습기간 평균 DA가격 {train_da_price.mean():.2f} 근처면 그 시점 기준선과 비슷한 가격이라는 뜻)")
...
# [계절 정규화] 여기서부터는 정규화된 가격(train_da_price_norm/train_rt_price_norm)을 쓴다
...
for i in range(n_train_obs):                                    # 3600개를 하나씩 순서대로
    penalty_i = PENALTY_RATE * train_da_price_norm[i]             # 이 표본의 부족분 벌금단가 (정규화된 DA가격 기준)
    surplus_cost[i] = (-W1 * train_rt_price_norm[i]) + W2   # [계절 정규화] 정규화된 RT가격 사용
...
objective_expr = (
    (-W1 * train_da_price_norm) @ x_var   # [계절 정규화] 정규화된 DA가격 사용
...
print()
print("=== 논문 제안 모형 MLR [계절 정규화](목적함수는 최근 30일 평균가 대비 비율, 평가는 원래 가격) - 블록18 결과 ===")
