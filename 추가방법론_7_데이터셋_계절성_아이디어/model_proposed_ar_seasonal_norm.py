...
# 0. 설정값
# =====================================================================
BASE_DIR = os.path.abspath(r"D:\Users\Downloads\데이터셋 계절성 아이디어")           # 이 파이썬 파일이 있는 폴더
...
# [계절 정규화] 학습 목적함수에 쓸 가격을 "최근 N일 평균 DA가격" 대비
# 비율로 바꿔서 씀 - 절대 가격 수준(계절에 따라 다름)이 아니라 상대적인
# 가격 신호로 학습하게 하려는 목적. 평가(6번 이후)는 원래 가격 그대로 씀.
BASELINE_WINDOW_DAYS = 30          # 기준선 계산에 쓸 과거 일수 (그 날짜 이전만 사용, 미래정보 없음)
...
# =====================================================================
# 3-B. [계절 정규화] 학습기간 각 날짜의 "최근 30일 평균 DA가격" 기준선을
#      계산하고, 학습에 쓸 정규화된 DA/RT가격을 만든다.
#      - 기준선은 그 날짜 "이전"의 daylight_table 전체(학습기간보다 훨씬
#        긴 과거 데이터)에서 뽑으므로 미래 정보를 쓰지 않는다.
#      - 테스트기간에는 이 정규화를 적용하지 않는다(평가는 원래 가격 그대로).
# =====================================================================

daily_avg_da_all = daylight_table.groupby("local_date")["da_price"].mean()  # 날짜별 낮시간 평균 DA가격 (전체 기간)
daily_avg_da_all = daily_avg_da_all.sort_index()                             # 날짜 오름차순 정렬

train_baseline = np.zeros(n_train_days)                    # 학습 날짜별 기준선(최근 30일 평균 DA가격)
for day_index in range(n_train_days):                         # 학습 날짜를 하나씩 순서대로
    this_date = train_dates_sorted[day_index]                    # 이 학습일의 날짜
    past_dates = daily_avg_da_all.index[daily_avg_da_all.index < this_date]   # 이 날짜보다 이전인 날짜들만
    recent_past_dates = past_dates[-BASELINE_WINDOW_DAYS:]           # 그중 가장 최근 30개
    train_baseline[day_index] = daily_avg_da_all.loc[recent_past_dates].mean()   # 그 30일의 평균 = 기준선

train_da_price_norm = np.zeros((n_train_days, HOURS_PER_DAY))   # 정규화된 DA가격(학습용, MILP 목적함수 전용)
train_rt_price_norm = np.zeros((n_train_days, HOURS_PER_DAY))   # 정규화된 RT가격(학습용, MILP 목적함수 전용)
reference_price_scale = train_da_price.mean()   # 학습기간 전체 평균 DA가격 - 정규화 후에도 W1/W2 균형이
                                                 # 원래와 비슷하게 유지되도록 스케일을 복원하는 배율
for day_index in range(n_train_days):                              # 학습 날짜를 하나씩 순서대로
    for hour in range(HOURS_PER_DAY):                                 # 시간대를 하나씩 순서대로
        train_da_price_norm[day_index, hour] = (train_da_price[day_index, hour] / train_baseline[day_index]) * reference_price_scale
        train_rt_price_norm[day_index, hour] = (train_rt_price[day_index, hour] / train_baseline[day_index]) * reference_price_scale

print()
print("[계절 정규화] 학습기간 기준선(최근 30일 평균 DA가격) 범위:",
      f"{train_baseline.min():.2f} ~ {train_baseline.max():.2f}")
print("[계절 정규화] 정규화된 DA가격 범위:",
      f"{train_da_price_norm.min():.4f} ~ {train_da_price_norm.max():.4f}",
      f"(참고: 학습기간 평균 DA가격 {reference_price_scale:.2f} 근처면 그 시점 기준선과 비슷한 가격이라는 뜻)")
...
for hour in range(HOURS_PER_DAY):                       # 시간대 0부터 11까지 순서대로 하나씩 처리
    ...
    da_this_hour = train_da_price[:, hour]                  # 이 시간대의 학습용 DA가격 300개 (원래 값 - 5-1 오라클 계산용)
    rt_this_hour = train_rt_price[:, hour]                  # 이 시간대의 학습용 RT가격 300개 (원래 값 - 5-1 오라클 계산용)
    da_this_hour_norm = train_da_price_norm[:, hour]        # [계절 정규화] 목적함수에 쓸 정규화된 DA가격
    rt_this_hour_norm = train_rt_price_norm[:, hour]        # [계절 정규화] 목적함수에 쓸 정규화된 RT가격
    ...
    # [계절 정규화] 여기서부터는 정규화된 가격(da_this_hour_norm/rt_this_hour_norm)을 쓴다
    surplus_cost = np.zeros(n_train_obs)                          # 잉여(y_plus) 1단위당 목적함수 계수
    shortage_cost = np.zeros(n_train_obs)                          # 부족(y_minus) 1단위당 목적함수 계수
    for i in range(n_train_obs):                                    # 300개를 하나씩 순서대로
        penalty_i = PENALTY_RATE * da_this_hour_norm[i]               # 이 표본의 부족분 벌금단가 (정규화된 DA가격 기준)
        surplus_cost[i] = (-W1 * rt_this_hour_norm[i]) + W2   # [계절 정규화] 정규화된 RT가격 사용
        shortage_cost[i] = (W1 * penalty_i) + W2              # [계절 정규화] 정규화된 벌금단가 사용
    ...
    objective_expr = (
        (-W1 * da_this_hour_norm) @ x_var   # [계절 정규화] 정규화된 DA가격 사용
...
print()
print("=== 논문 제안 모형 AR [계절 정규화](목적함수는 최근 30일 평균가 대비 비율, 평가는 원래 가격) - z03 블록18 결과 ===")
