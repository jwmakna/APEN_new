...
def compute_gap_4term(pred_flat, penalty_rate, actual, da, rt):
    """방법6 조건부 이익함수(PC=rate*DA*I[DA>RT]+RT) optimality gap 계산 (oracle: {0, actual, 1.0} 3후보)

    [gap정정] 원래 이 함수는 방법2(4항)의 compute_gap을 그대로 가져다 썼는지 PC 항에
    I[DA>RT] indicator가 빠져 있었다 - 즉 DA<=RT인 관측치에서도 항상 RT+PC를 부족비용으로
    썼다. 그런데 학습 목적함수(solve_ar_proposed/solve_mlr_proposed의 yc = W1*RT +
    W1*PC*indicator + W2_eff)는 처음부터 조건부였으므로, 학습과 평가(oracle/realized/gap)가
    서로 다른 이익식을 쓰는 불일치였다. 아래에서 indicator를 pc에 곱해 학습 쪽과 동일하게
    맞춘다 - 나머지 계산 구조(오라클 3후보, scale 등)는 전혀 바꾸지 않았다."""
    sum_realized = 0.0
    sum_oracle = 0.0
    for i in range(len(pred_flat)):
        a = actual[i]; x = pred_flat[i]
        dp = da[i]; rp = rt[i]
        indicator = 1.0 if dp > rp else 0.0     # [gap정정] I[DA>RT] - 학습 목적함수와 동일한 조건
        pc = penalty_rate * dp * indicator      # [gap정정] PC = rate*DA*I[DA>RT] (조건부로 정정)
        mismatch = a - x
...
grid_csv = os.path.join(OUT_DIR, "grid_method6_ar_mlr_z03_block18_evaluation수정.csv")
...
save_csv(os.path.join(OUT_DIR, "fig3_method6_AR_evaluation수정.csv"),
         ["Label", "nRMSE", "Gap"],
         [[FIG_LABELS[i], round(fig3_ar_n[i], 2), round(fig3_ar_g[i], 2)]
          for i in range(len(FIG_LABELS))])

save_csv(os.path.join(OUT_DIR, "fig5_method6_AR_evaluation수정.csv"),
...
save_csv(os.path.join(OUT_DIR, "fig6_method6_MLR_evaluation수정.csv"),
         ["Label", "nRMSE", "Gap"],
         [[FIG_LABELS[i], round(fig6_mlr_n[i], 2), round(fig6_mlr_g[i], 2)]
          for i in range(len(FIG_LABELS))])

save_csv(os.path.join(OUT_DIR, "fig8_method6_MLR_evaluation수정.csv"),
...
fig.tight_layout()
p3 = os.path.join(RESULTS_DIR, "방법6_fig3_ar_z03_block18_evaluation수정.png")
...
p5 = os.path.join(RESULTS_DIR, "방법6_fig5_ar_z03_block18_evaluation수정.png")
...
p6 = os.path.join(RESULTS_DIR, "방법6_fig6_mlr_z03_block18_evaluation수정.png")
...
p8 = os.path.join(RESULTS_DIR, "방법6_fig8_mlr_z03_block18_evaluation수정.png")
