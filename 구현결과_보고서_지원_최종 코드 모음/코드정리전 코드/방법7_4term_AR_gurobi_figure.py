...
# =====================================================================
# 방법7_4term_AR_gurobi_figure.py
#
# 목적: "방법7 - Additive CVaR" (보고서 3.2.7절 수식)을 4항 profit_change(PC=RT+rate*DA) 수익식
#       위에 얹은 AR(자기회귀) 모형의 nRMSE·optimality gap과, 시간대별/경제적
#       regret 지표까지 구한다. 회귀 계수를 예측오차만 보고 정하지 않고,
#       "경제적 regret"과 "regret 상위 10%(CVaR90)"를 함께 최소화하도록
#       Gurobi MILP로 학습한다(figure 버전).
#
...
# 구간(z03 블록18): 학습 2013-08-25~2013-11-22(300일),
#                  테스트 2013-11-23~2013-12-22(100일)
...
# 목적함수 = 기존 W2*sum(abs error) + W1*sum(regret)  [항상 그대로]
#          + W1 * CVAR_LAMBDA * (N*zeta + sum(s)/(1-CVAR_ALPHA))  [CVaR 항을 추가로 더함]
CVAR_ALPHA = 0.90
CVAR_LAMBDA = 0.05
...
# ---- 시간대별 학습용 오라클 (CVaR 항에만 필요, W1/W2 스윕과 무관하므로 미리 한 번만 계산) ----
# PENALTY_RATE(3항, swept 아님)만 쓰므로 W1/W2 비율 루프 밖에서 12시간치를 한 번만 구한다.
profit_scale = CAPACITY_MW * DURATION_HOURS
oracle_profit_train_unscaled_by_hour = np.zeros((HOURS_PER_DAY, n_train_obs))
for hour in range(HOURS_PER_DAY):
    y_this_hour_oc = train_solar[:, hour]
    da_this_hour_oc = train_da_price[:, hour]
    rt_this_hour_oc = train_rt_price[:, hour]
...
for ratio_label, sweep_w1, sweep_w2 in RATIO_LIST:
    ...
    coefficients_by_hour_sw_cvar = np.zeros((HOURS_PER_DAY, n_features))
    ...
    for hour in range(HOURS_PER_DAY):
        ...
        # ================================================================
        # CVaR(λ=0.05) 버전: 위와 완전히 같은 제약(잉여/부족/이진 complementarity)에
        # zeta·s_var·CVaR 항만 추가한다. 목적함수의 mean-regret 부분은 그대로 두고
        # (기존 규모 유지) CVaR 항을 W1(=sweep_w1)에 비례해 더한다.
        # ================================================================
        gmodel_sw_cvar = gp.Model(f"proposed_ar_sweep_cvar_{ratio_label}_{hour}")
        ...
        gmodel_sw_cvar.addConstr(x_var_swc - ar_design_matrix @ beta_var_swc == 0.0)
        gmodel_sw_cvar.addConstr(x_var_swc + yplus_var_swc - yminus_var_swc == y_this_hour)
        ...
        shortage_price_this_hour_swc = PENALTY_RATE * da_this_hour + rt_this_hour   # 4항 부족비용(벌금+RT가격)
        oracle_this_hour = oracle_profit_train_unscaled_by_hour[hour]

        for i in range(n_train_obs):
            profit_expression_i = (
                da_this_hour[i] * x_var_swc[i]
                + rt_this_hour[i] * yplus_var_swc[i]
                - shortage_price_this_hour_swc[i] * yminus_var_swc[i]
            )
        ...
        total_regret_objective_swc = (
            (-sweep_w1 * da_this_hour) @ x_var_swc
        ...
        coefficients_by_hour_sw_cvar[hour] = beta_var_swc.X
    ...
    # ---- CVaR(λ=0.05) 버전 예측·평가 (위와 완전히 같은 절차, coefficients만 다름) ----
    test_forecast_sw_cvar = np.zeros((n_test_days, HOURS_PER_DAY))
    previous_day_actual_swc = train_solar[-1]
    for day_index in range(n_test_days):
        feature_vector_swc = np.concatenate([[1.0], previous_day_actual_swc[::-1]])
        for hour in range(HOURS_PER_DAY):
            raw_prediction = np.dot(coefficients_by_hour_sw_cvar[hour], feature_vector_swc)
            test_forecast_sw_cvar[day_index, hour] = min(max(raw_prediction, 0.0), 1.0)
        previous_day_actual_swc = test_solar[day_index]

    predicted_flat_swc = test_forecast_sw_cvar.flatten()

    sse_swc = 0.0
    for i in range(len(actual_flat_sw)):
        e = actual_flat_sw[i] - predicted_flat_swc[i]
        sse_swc = sse_swc + e * e
    rmse_swc = (sse_swc / len(actual_flat_sw)) ** 0.5
    ...
    for i in range(len(actual_flat_sw)):
        actual_i = actual_flat_sw[i]
        commitment_i = predicted_flat_swc[i]
        da_i = da_flat_sw[i]
        rt_i = rt_flat_sw[i]
        penalty_cost_i = PENALTY_RATE * da_i
        mismatch_i = actual_i - commitment_i
    ...
    print(f"[{ratio_label:>8}] (CVaR λ=0.05) nRMSE={nrmse_swc:.4f}%  gap={gap_swc:.4f}%")
...
#     CVaR(λ=0.05) 적용 전/후를 겹쳐서 비교한다 (논문 참조곡선은 생략)
# =====================================================================
...
plt.title("Fig.3 (4항 profit_change) CVaR 적용 전/후 비교 - z03 블록18")
plt.tight_layout()
plt.savefig("fig3_ar_profit_change_cvar_compare.png", dpi=150)
print("저장 완료: fig3_ar_profit_change_cvar_compare.png")
plt.show()
...
for penalty_rate_sw in PENALTY_LIST:
    ...
    coefficients_by_hour_p5_cvar = np.zeros((HOURS_PER_DAY, n_features))
    for hour in range(HOURS_PER_DAY):
        y_this_hour = train_solar[:, hour]
        da_this_hour = train_da_price[:, hour]
        rt_this_hour = train_rt_price[:, hour]
        ...
        # ================================================================
        # CVaR(λ=0.05) 버전: 오라클도 이번엔 PENALTY_RATE 대신 penalty_rate_sw로
        # 계산해야 한다 (여기서는 벌금율 자체가 스윕 대상이라 오라클도 같이 바뀜).
        # ================================================================
        oracle_profit_train_unscaled_p5 = np.zeros(n_train_obs)
        for i in range(n_train_obs):
            actual_i = y_this_hour[i]
            da_i = da_this_hour[i]
            rt_i = rt_this_hour[i]
            ...
            oracle_i = max(profit_commit_0, profit_commit_actual, profit_commit_1)
            oracle_profit_train_unscaled_p5[i] = oracle_i / profit_scale

        gmodel_p5_cvar = gp.Model(f"proposed_ar_fig5_cvar_{penalty_rate_sw}_{hour}")
        gmodel_p5_cvar.Params.OutputFlag = 0
        gmodel_p5_cvar.Params.MIPGap = 1e-9
        gmodel_p5_cvar.Params.TimeLimit = 60

        beta_var_p5c = gmodel_p5_cvar.addMVar(n_features, lb=-GRB.INFINITY, name="beta")
        x_var_p5c = gmodel_p5_cvar.addMVar(n_train_obs, lb=0.0, ub=1.0, name="x")
        yplus_var_p5c = gmodel_p5_cvar.addMVar(n_train_obs, lb=0.0, ub=1.0, name="y_plus")
        yminus_var_p5c = gmodel_p5_cvar.addMVar(n_train_obs, lb=0.0, ub=1.0, name="y_minus")

        gmodel_p5_cvar.addConstr(x_var_p5c - ar_design_matrix @ beta_var_p5c == 0.0)
        gmodel_p5_cvar.addConstr(x_var_p5c + yplus_var_p5c - yminus_var_p5c == y_this_hour)

        if n_binary_p5 > 0:
            z_var_p5c = gmodel_p5_cvar.addMVar(n_binary_p5, vtype=GRB.BINARY, name="z")
            gmodel_p5_cvar.addConstr(yplus_var_p5c[binary_rows_p5] + z_var_p5c <= 1.0)
            gmodel_p5_cvar.addConstr(yminus_var_p5c[binary_rows_p5] - z_var_p5c <= 0.0)

        zeta_var_p5c = gmodel_p5_cvar.addVar(lb=0.0, name="cvar_zeta")
        s_var_p5c = gmodel_p5_cvar.addMVar(n_train_obs, lb=0.0, name="cvar_excess")

        shortage_price_this_hour_p5c = penalty_rate_sw * da_this_hour + rt_this_hour   # 4항 부족비용(벌금+RT가격), 이번엔 penalty_rate_sw 사용

        for i in range(n_train_obs):
            profit_expression_i = (
                da_this_hour[i] * x_var_p5c[i]
                + rt_this_hour[i] * yplus_var_p5c[i]
                - shortage_price_this_hour_p5c[i] * yminus_var_p5c[i]
            )
            regret_expression_i = oracle_profit_train_unscaled_p5[i] - profit_expression_i
            gmodel_p5_cvar.addConstr(
                s_var_p5c[i] >= regret_expression_i - zeta_var_p5c,
                name=f"cvar_tail_{i}"
            )

        total_regret_objective_p5c = (
            (-FIG5_W1 * da_this_hour) @ x_var_p5c
            + surplus_cost_p5 @ yplus_var_p5c
            + shortage_cost_p5 @ yminus_var_p5c
        )
        cvar_objective_p5c = (
            FIG5_W1 * CVAR_LAMBDA
            * (n_train_obs * zeta_var_p5c + s_var_p5c.sum() / (1.0 - CVAR_ALPHA))
        )
        gmodel_p5_cvar.setObjective(total_regret_objective_p5c + cvar_objective_p5c, GRB.MINIMIZE)
        gmodel_p5_cvar.optimize()

        coefficients_by_hour_p5_cvar[hour] = beta_var_p5c.X
    ...
    # ---- CVaR(λ=0.05) 버전 예측·평가 (위와 같은 절차, coefficients만 다름) ----
    proposed_forecast_p5_cvar = np.zeros((n_test_days, HOURS_PER_DAY))
    previous_day_actual_p5c = train_solar[-1]
    for day_index in range(n_test_days):
        feature_vector_p5c = np.concatenate([[1.0], previous_day_actual_p5c[::-1]])
        for hour in range(HOURS_PER_DAY):
            raw_prediction = np.dot(coefficients_by_hour_p5_cvar[hour], feature_vector_p5c)
            proposed_forecast_p5_cvar[day_index, hour] = min(max(raw_prediction, 0.0), 1.0)
        previous_day_actual_p5c = test_solar[day_index]

    proposed_predicted_flat_p5c = proposed_forecast_p5_cvar.flatten()

    sse_p5c = 0.0
    for i in range(len(actual_flat_f5)):
        e = actual_flat_f5[i] - proposed_predicted_flat_p5c[i]
        sse_p5c = sse_p5c + e * e
    nrmse_p5c = 100.0 * ((sse_p5c / len(actual_flat_f5)) ** 0.5) / avg_actual_f5

    sum_realized_p5c = 0.0
    sum_oracle_p5c = 0.0
    for i in range(len(actual_flat_f5)):
        actual_i = actual_flat_f5[i]
        commitment_i = proposed_predicted_flat_p5c[i]
        da_i = da_flat_f5[i]
        rt_i = rt_flat_f5[i]
        penalty_cost_i = penalty_rate_sw * da_i
        mismatch_i = actual_i - commitment_i
        ...
        sum_realized_p5c = sum_realized_p5c + realized_profit_i
        ...
        oracle_profit_i = max(profit_if_commit_zero, profit_if_commit_actual, profit_if_commit_full)
        sum_oracle_p5c = sum_oracle_p5c + oracle_profit_i

    gap_p5c = 100.0 * (sum_oracle_p5c - sum_realized_p5c) / sum_oracle_p5c

    proposed_nrmse_list_cvar.append(nrmse_p5c)
    proposed_gap_list_cvar.append(gap_p5c)

    print(
        f"[벌금율 {penalty_rate_sw*100:.0f}%] AR nRMSE={ar_nrmse_f5:.4f}% gap={gap_ar_sw:.4f}% "
        f"| Proposed nRMSE={nrmse_p5:.4f}% gap={gap_p5:.4f}% "
        f"| Proposed+CVaR nRMSE={nrmse_p5c:.4f}% gap={gap_p5c:.4f}%"
    )

penalty_pct_labels = [f"{int(p*100)}%" for p in PENALTY_LIST]

# CVaR(λ=0.05) 적용 전/후 Proposed 곡선을 겹쳐 그린다 (논문 참조곡선은 생략, AR 기본선은 유지)
fig, (ax_a, ax_b) = plt.subplots(1, 2, figsize=(12, 5))
ax_a.plot(penalty_pct_labels, ar_nrmse_list, color="#3B82F6", marker="o", label="AR (기본)")
...
ax_b.plot(penalty_pct_labels, ar_gap_list, color="#3B82F6", marker="o", label="AR (기본)")
...
plt.suptitle("Fig.5 (4항 profit_change) CVaR 적용 전/후 비교 - z03 블록18 (W1=W2=1)")
plt.tight_layout()
plt.savefig("fig5_ar_profit_change_cvar_compare.png", dpi=150)
print("저장 완료: fig5_ar_profit_change_cvar_compare.png")
plt.show()