...
# =====================================================================
# 4항 수익식: 시간대별 예측오차 + 경제적 regret 통합 분석
#
# 4항 수익식:
# profit = DA*x + RT*surplus - (PC+RT)*shortage
# =====================================================================

import matplotlib.pyplot as plt

# Baseline AR에서 사용
#MODEL_LABEL = "Baseline AR (4-term)"
#OUTPUT_TAG = "baseline_ar_4term"

# Proposed AR에서는 위 두 줄 대신 아래 두 줄 사용
MODEL_LABEL = "Proposed AR (4-term)"
OUTPUT_TAG = "proposed_ar_4term"
...
hourly_rmse_capacity_percent = (
    np.sqrt(
        np.mean(forecast_error ** 2, axis=0)
    )
    * 100.0
)
...
penalty_matrix = (
    PENALTY_RATE * da_matrix
)

mismatch_matrix = (
    actual_matrix - commitment_matrix
)

surplus_matrix = np.maximum(
    mismatch_matrix,
    0.0
)

shortage_matrix = np.maximum(
    -mismatch_matrix,
    0.0
)
...
# S>1이면 x=S는 허용범위를 벗어나므로 후보에서 제외
profit_commit_actual = np.where(
    actual_matrix <= 1.0,
    profit_commit_actual,
    -np.inf
)
...
# 후보 3: x=1
surplus_if_full = np.maximum(
    actual_matrix - 1.0,
    0.0
)

shortage_if_full = np.maximum(
    1.0 - actual_matrix,
    0.0
)
...
if minimum_regret < -1e-8:
    ...
    raise ValueError(
        "음수 regret이 발견됐습니다. "
        "4항 수익식 또는 오라클 계산을 확인해야 합니다."
    )

# 부동소수점 오차 수준의 작은 값은 0으로 정리
economic_regret[
