...
from model_baseline_ar import PENALTY_RATE                     # Gurobi 상수
...
# CVaR 신뢰수준: regret이 큰 최악의 10%를 관리
CVAR_ALPHA = 0.90

# CVaR 가중치
# 0.0이면 기존 APEN, 0보다 크면 CVaR 활성화
CVAR_LAMBDA = 0.10
...
if not (0.0 <= CVAR_LAMBDA <= 1.0):
    raise ValueError("CVAR_LAMBDA는 0과 1 사이여야 합니다.")
...
for hour in range(HOURS_PER_DAY):                       # 시간대 0부터 11까지 순서대로 하나씩 처리
    ...
    #
    # N * CVaR_alpha
    # = N*zeta + 1/(1-alpha)*sum(s_i)
    # ================================================================

    mean_regret_objective = (
        (-mean_regret_weight * W1 * da_this_hour)
        @ x_var
...
# =====================================================================
# Zhang 비대칭가격 수익식: 시간대별 예측오차 + 경제적 regret 통합 분석
#
# Zhang 수익식 (3항):
# penalty = ZHANG_C * RT
# profit = DA*x + RT*surplus - penalty*shortage
# =====================================================================

import matplotlib.pyplot as plt

# Baseline AR에서 사용
#MODEL_LABEL = "Baseline AR (Zhang)"
#OUTPUT_TAG = "baseline_ar_zhang"

# Proposed AR에서는 위 두 줄 대신 아래 두 줄 사용
MODEL_LABEL = "Proposed AR (Zhang)"
OUTPUT_TAG = "proposed_ar_zhang"
...
actual_matrix = test_solar
commitment_matrix = test_forecast
da_matrix = test_da_price
rt_matrix = test_rt_price

penalty_matrix = ZHANG_C * rt_matrix   # [4항 코드와 다른 부분] PENALTY_RATE*da_matrix -> ZHANG_C*rt_matrix

mismatch_matrix = actual_matrix - commitment_matrix
surplus_matrix = np.maximum(mismatch_matrix, 0.0)
shortage_matrix = np.maximum(-mismatch_matrix, 0.0)

# Zhang 수익식 (3항, RT 구매비용 항 없음)
economic_realized_profit = (
    CAPACITY_MW * DURATION_HOURS
    * (
        da_matrix * commitment_matrix
        + rt_matrix * surplus_matrix
        - penalty_matrix * shortage_matrix
    )
)


# =====================================================================
# 3. Zhang 수익식의 오라클 수익 (후보 x=0, x=S, x=1)
# =====================================================================

profit_commit_zero = CAPACITY_MW * DURATION_HOURS * rt_matrix * actual_matrix

profit_commit_actual = CAPACITY_MW * DURATION_HOURS * da_matrix * actual_matrix
profit_commit_actual = np.where(actual_matrix <= 1.0, profit_commit_actual, -np.inf)

surplus_if_full = np.maximum(actual_matrix - 1.0, 0.0)
shortage_if_full = np.maximum(1.0 - actual_matrix, 0.0)

profit_commit_full = (
    CAPACITY_MW * DURATION_HOURS
    * (
        da_matrix
        + rt_matrix * surplus_if_full
        - penalty_matrix * shortage_if_full
    )
)

economic_oracle_profit = np.maximum.reduce([
    profit_commit_zero,
...
# =====================================================================
# 4. 시점별 economic regret
# =====================================================================

economic_regret = economic_oracle_profit - economic_realized_profit
...
if minimum_regret < -1e-8:
    raise ValueError("음수 regret이 발견됐습니다. Zhang 수익식 또는 오라클 계산을 확인해야 합니다.")
...
oracle_sum_by_hour = np.sum(economic_oracle_profit, axis=0)

hourly_gap_percent = np.full(HOURS_PER_DAY, np.nan)
np.divide(
    100.0 * regret_sum_by_hour,
    oracle_sum_by_hour,
    out=hourly_gap_percent,
    where=np.abs(oracle_sum_by_hour) > 1e-12
)


# =====================================================================
# 6. 기존 전체 결과와 일치하는지 검증
# =====================================================================

overall_nrmse_from_hourly = (
    100.0 * np.sqrt(np.mean((hourly_rmse_capacity_percent / 100.0) ** 2)) / np.mean(test_solar)
)

overall_gap_from_regret = (
    100.0 * np.sum(economic_regret) / np.sum(economic_oracle_profit)
)
...
print(f"regret으로 재계산한 gap      = {overall_gap_from_regret:.6f}%")
...
print()
print(f"=== {MODEL_LABEL}: 시간대별 통합 결과 ===")
...
# =====================================================================
# 10. CVaR-APEN Table 3 스윕
#
# CVaR_ALPHA, CVAR_LAMBDA는 고정하고
# W1/W2 비율만 논문 Table 3과 동일하게 변경한다.
#
# 수익식: zhang
# 
# =====================================================================
...
sweep_cvar90_regret = []
sweep_max_regret = []
sweep_realized_profit = []


print()
print("========================================================")
print("CVaR-APEN Table 3 스윕 시작")
print(
    f"4항 수익식, "
    f"alpha={CVAR_ALPHA}, "
    f"lambda={CVAR_LAMBDA}"
...
for ratio_label, sweep_w1, sweep_w2 in RATIO_LIST:
    ...
    print(
        f"W1/W2 = {ratio_label} "
        f"(W1={sweep_w1}, W2={sweep_w2})"
    )
    print("----------------------------------------")

    coefficients_by_hour_sw = np.zeros(
        (HOURS_PER_DAY, n_features)
    )


    # =================================================================
    # 1. 12개 시간대별 CVaR-AR 학습
    # =================================================================
    ...
    for hour in range(HOURS_PER_DAY):
        ...
        # 1-1. 학습용 오라클
        #
        # 여기서는 기존 APEN 목적함수와 스케일을 맞추기 위해
        # CAPACITY_MW=30을 곱하지 않은 수익을 사용한다.
        # -------------------------------------------------------------

        profit_commit_zero_train = (
            rt_this_hour * y_this_hour
        )

        profit_commit_actual_train = (
            da_this_hour * y_this_hour
        )

        profit_commit_actual_train = np.where(
            y_this_hour <= 1.0,
            profit_commit_actual_train,
            -np.inf
        )

        surplus_if_full_train = np.maximum(
            y_this_hour - 1.0,
            0.0
        )

        shortage_if_full_train = np.maximum(
            1.0 - y_this_hour,
            0.0
        )

        profit_commit_full_train = (
            da_this_hour
            + rt_this_hour
            * surplus_if_full_train
            - shortage_price_this_hour
            * shortage_if_full_train
        )

        oracle_profit_train_unscaled = np.maximum.reduce([
            profit_commit_zero_train,
            profit_commit_actual_train,
            profit_commit_full_train
        ])
        ...
        safe_ratio_label = ratio_label.replace(
            "/",
            "_"
        )

        gmodel_sw = gp.Model(
            f"cvar_4term_table3_"
            f"{safe_ratio_label}_hour_{hour}"
        )

        gmodel_sw.Params.OutputFlag = 0
        gmodel_sw.Params.MIPGap = 1e-9

        beta_var_sw = gmodel_sw.addMVar(
            n_features,
            lb=-GRB.INFINITY,
            name="beta"
        )

        x_var_sw = gmodel_sw.addMVar(
            n_train_obs,
            lb=0.0,
            ub=1.0,
            name="x"
        ...
        yplus_var_sw = gmodel_sw.addMVar(
            n_train_obs,
            lb=0.0,
            ub=1.0,
            name="y_plus"
        ...
        yminus_var_sw = gmodel_sw.addMVar(
            n_train_obs,
            lb=0.0,
            ub=1.0,
            name="y_minus"
        ...
        # surplus와 shortage의 동시 발생 방지
        gmodel_sw.addConstr(
            yplus_var_sw + z_var_sw <= 1.0,
            name="surplus_only"
        )

        gmodel_sw.addConstr(
            yminus_var_sw - z_var_sw <= 0.0,
            name="shortage_only"
        )
        ...
        zeta_var_sw = gmodel_sw.addVar(
            lb=0.0,
            name="cvar_zeta"
        )

        s_var_sw = gmodel_sw.addMVar(
            n_train_obs,
            lb=0.0,
            name="cvar_excess"
        )
        ...
        cvar_objective_sw = (
            ...
            * CVAR_LAMBDA
            * (
                n_train_obs * zeta_var_sw
                + s_var_sw.sum()
                / (1.0 - CVAR_ALPHA)
            )
        )

        objective_expression_sw = (
            mean_regret_objective_sw
            + cvar_objective_sw
        )

        gmodel_sw.setObjective(
            objective_expression_sw,
            GRB.MINIMIZE
        )
        ...
        if gmodel_sw.Status != GRB.OPTIMAL:

            raise RuntimeError(
                f"Table 3 스윕 실패: "
                f"W1/W2={ratio_label}, "
                f"hour={hour}, "
                f"status={gmodel_sw.Status}"
            )

        coefficients_by_hour_sw[hour] = (
            beta_var_sw.X
        )

        print(
            f"  시간대 {hour + LOCAL_HOUR_START}시 완료"
        )
    ...
    test_forecast_sw = np.zeros(
        (n_test_days, HOURS_PER_DAY)
    )

    previous_day_actual_sw = (
        train_solar[-1]
    )

    for day_index in range(n_test_days):

        feature_vector_sw = np.concatenate([
            [1.0],
            previous_day_actual_sw[::-1]
        ])

        for hour in range(HOURS_PER_DAY):

            raw_prediction_sw = np.dot(
                coefficients_by_hour_sw[hour],
                feature_vector_sw
            )
            ...
            test_forecast_sw[
                ...
                max(raw_prediction_sw, 0.0),
                1.0
            )

        previous_day_actual_sw = (
            test_solar[day_index]
        )
    ...
    penalty_matrix_sw = (
        PENALTY_RATE * test_da_price
    )

    shortage_price_matrix_sw = (
        ZHANG_C * test_rt_price
    )

    mismatch_matrix_sw = (
        test_solar - test_forecast_sw
    )

    surplus_matrix_sw = np.maximum(
        mismatch_matrix_sw,
        0.0
    )

    shortage_matrix_sw = np.maximum(
        -mismatch_matrix_sw,
        0.0
    )

    realized_profit_matrix_sw = (
        CAPACITY_MW
        * DURATION_HOURS
        * (
            test_da_price
    ...
    profit_commit_actual_sw = np.where(
        test_solar <= 1.0,
        profit_commit_actual_sw,
        -np.inf
    )

    surplus_if_full_sw = np.maximum(
        test_solar - 1.0,
        0.0
    )

    shortage_if_full_sw = np.maximum(
        1.0 - test_solar,
        0.0
    )

    profit_commit_full_sw = (
        CAPACITY_MW
        * DURATION_HOURS
        * (
            test_da_price
    ...
    oracle_profit_matrix_sw = np.maximum.reduce([
        profit_commit_zero_sw,
        profit_commit_actual_sw,
        profit_commit_full_sw
    ])
    ...
    # =================================================================
    # 6. 테스트 regret, gap, CVaR90
    # =================================================================

    regret_matrix_sw = (
        oracle_profit_matrix_sw
        - realized_profit_matrix_sw
    )

    minimum_regret_sw = np.min(
        regret_matrix_sw
    )

    if minimum_regret_sw < -1e-8:

        raise ValueError(
            f"W1/W2={ratio_label}에서 "
            f"음수 regret 발생: "
            f"{minimum_regret_sw}"
        )

    regret_matrix_sw[
        np.abs(regret_matrix_sw) < 1e-8
    ] = 0.0

    total_realized_profit_sw = np.sum(
        realized_profit_matrix_sw
    )

    total_oracle_profit_sw = np.sum(
        oracle_profit_matrix_sw
    )

    gap_sw = (
        100.0
        * np.sum(regret_matrix_sw)
        / total_oracle_profit_sw
    )

    mean_regret_sw = np.mean(
        regret_matrix_sw
    )

    regret_flat_sw = (
        regret_matrix_sw.flatten()
    )
    ...
    sorted_regret_sw = np.sort(
        regret_flat_sw
    )

    cvar90_regret_sw = np.mean(
        sorted_regret_sw[
            -tail_count_sw:
        ]
    )

    max_regret_sw = np.max(
        regret_flat_sw
    )
    ...
    sweep_mean_regret.append(
        mean_regret_sw
    )

    sweep_cvar90_regret.append(
        cvar90_regret_sw
    )

    sweep_max_regret.append(
        max_regret_sw
    )

    sweep_realized_profit.append(
        total_realized_profit_sw
    )
    ...
    print(
        ...
        f"nRMSE={nrmse_sw:.4f}% | "
        f"gap={gap_sw:.4f}% | "
        f"mean regret={mean_regret_sw:.4f} | "
        f"CVaR90={cvar90_regret_sw:.4f} | "
        f"max regret={max_regret_sw:.4f} | "
        f"profit={total_realized_profit_sw:.4f}"
    )
...
table3_cvar_result = pd.DataFrame({
    "W1_W2":
        sweep_labels,
    ...
    "CVAR_alpha":
        [CVAR_ALPHA] * len(sweep_labels),

    "CVAR_lambda":
        [CVAR_LAMBDA] * len(sweep_labels),
})
...
print("========================================================")
print(
    "CVaR-APEN Table 3 결과 "
    f"(4항, alpha={CVAR_ALPHA}, "
    f"lambda={CVAR_LAMBDA})"
)
print("========================================================")
...
table3_csv_name = (
    f"table3_cvar_ar_zhang_"
    f"alpha{alpha_tag_table3}_"
    f"lambda{lambda_tag_table3}.csv"
)

table3_cvar_result.to_csv(
    table3_csv_name,
    index=False,
    encoding="utf-8-sig"
)
