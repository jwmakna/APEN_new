...
baseline_result = np.load(
    "baseline_ar_zhang_combined_diagnostics.npz"
)

proposed_result = np.load(
    "proposed_ar_zhang_combined_diagnostics.npz"
)
...
comparison_table.to_csv(
    "ar_zhang_hourly_combined_comparison.csv",
    index=False,
    encoding="utf-8-sig"
)
...
axes[0, 0].set_title("(a) Hourly RMSE — Zhang")
...
axes[1, 1].set_title(
    "(d) Economic-Regret Change — Zhang\n"
    "Proposed AR - Baseline AR"
)
...
plt.savefig(
    "ar_zhang_hourly_combined_comparison.png",
    dpi=200,
    bbox_inches="tight"
)
