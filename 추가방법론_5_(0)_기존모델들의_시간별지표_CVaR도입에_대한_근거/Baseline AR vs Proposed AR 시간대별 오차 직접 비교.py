# =====================================================================
# Baseline AR vs Proposed AR
# 시간대별 예측오차와 경제적 regret 통합 비교
# =====================================================================
...
baseline_result = np.load(
    "baseline_ar_combined_diagnostics.npz"
)

proposed_result = np.load(
    "proposed_ar_combined_diagnostics.npz"
)
...
# 음수이면 Proposed AR의 경제적 regret 개선
mean_regret_change = (
...
baseline_regret_sum_by_hour = np.sum(
    baseline_regret,
    axis=0
)

proposed_regret_sum_by_hour = np.sum(
    proposed_regret,
    axis=0
)
...
for hour_index in range(len(local_hours)):

    if abs(
        oracle_sum_by_hour[hour_index]
    ) > 1e-12:

        baseline_gap_by_hour[hour_index] = (
            100.0
            * baseline_regret_sum_by_hour[hour_index]
            / oracle_sum_by_hour[hour_index]
        )

        proposed_gap_by_hour[hour_index] = (
            100.0
            * proposed_regret_sum_by_hour[hour_index]
            / oracle_sum_by_hour[hour_index]
        )
...
comparison_table.to_csv(
    "ar_hourly_combined_comparison.csv",
    index=False,
    encoding="utf-8-sig"
)
...
axes[0, 0].set_title("(a) Hourly RMSE")
axes[0, 0].grid(alpha=0.3)
axes[0, 0].legend()


# ---------------------------------------------------------------------
# (b) 시간대별 평균 경제적 regret 비교
# ---------------------------------------------------------------------
...
axes[0, 1].set_title("(b) Mean Economic Regret")
...
plt.savefig(
    "ar_hourly_combined_comparison.png",
    dpi=200,
    bbox_inches="tight"
)
