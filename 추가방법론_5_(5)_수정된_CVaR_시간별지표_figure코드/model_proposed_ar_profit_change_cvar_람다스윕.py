...
# 0. 설정값
# =====================================================================
BASE_DIR = os.path.abspath(r"D:\Users\Downloads\Github\추가방법론_5_(5)_수정된_CVaR_시간별지표_figure코드")           # 이 파이썬 파일이 있는 폴더
...
# =====================================================================
# 10-1. 5.7.3절용 λ 스윕 (W1=1, W2=20 고정, λ만 0~0.10로 변화)
#    - 위에서 이미 계산해 둔 oracle_profit_train_unscaled_by_hour를 그대로 재사용한다
# =====================================================================

LAMBDA_SWEEP_LIST = [0.0, 0.025, 0.05, 0.075, 0.10]   # 5.7.3절 표와 동일한 5개 값
lambda_sweep_results = []                              # {lambda, nrmse, gap, total_regret, cvar90, max_regret} 담을 리스트

for lam in LAMBDA_SWEEP_LIST:

    coefficients_by_hour_lam = np.zeros((HOURS_PER_DAY, n_features))
    ...
    for hour in range(HOURS_PER_DAY):
        ...
        surplus_cost_lam = np.zeros(n_train_obs)
        shortage_cost_lam = np.zeros(n_train_obs)
        for i in range(n_train_obs):
            penalty_i = PENALTY_RATE * da_this_hour[i]
            surplus_cost_lam[i] = (-W1 * rt_this_hour[i]) + W2
            shortage_cost_lam[i] = (W1 * (penalty_i + rt_this_hour[i])) + W2

        binary_row_list_lam = []
        for i in range(n_train_obs):
            if surplus_cost_lam[i] + shortage_cost_lam[i] < 0.0:
                binary_row_list_lam.append(i)
        binary_rows_lam = np.array(binary_row_list_lam, dtype=int)
        n_binary_lam = len(binary_rows_lam)

        gmodel_lam = gp.Model(f"lambda_sweep_h{hour}_{lam}")
        gmodel_lam.Params.OutputFlag = 0
        gmodel_lam.Params.MIPGap = 1e-9

        beta_lam = gmodel_lam.addMVar(n_features, lb=-GRB.INFINITY, name="beta")
        x_lam = gmodel_lam.addMVar(n_train_obs, lb=0.0, ub=1.0, name="x")
        yplus_lam = gmodel_lam.addMVar(n_train_obs, lb=0.0, ub=1.0, name="y_plus")
        yminus_lam = gmodel_lam.addMVar(n_train_obs, lb=0.0, ub=1.0, name="y_minus")

        gmodel_lam.addConstr(x_lam - ar_design_matrix @ beta_lam == 0.0)
        gmodel_lam.addConstr(x_lam + yplus_lam - yminus_lam == y_this_hour)

        if n_binary_lam > 0:
            z_lam = gmodel_lam.addMVar(n_binary_lam, vtype=GRB.BINARY, name="z")
            gmodel_lam.addConstr(yplus_lam[binary_rows_lam] + z_lam <= 1.0)
            gmodel_lam.addConstr(yminus_lam[binary_rows_lam] - z_lam <= 0.0)

        # ---- mean-regret 항 (기존 목적함수와 동일) ----
        mean_regret_obj_lam = (
            (-W1 * da_this_hour) @ x_lam
            + surplus_cost_lam @ yplus_lam
            + shortage_cost_lam @ yminus_lam
        )

        if lam > 0.0:
            # ---- CVaR 항 (5.7.2절 최종 Additive 식 그대로) ----
            zeta_lam = gmodel_lam.addVar(lb=0.0, name="cvar_zeta")
            s_lam = gmodel_lam.addMVar(n_train_obs, lb=0.0, name="cvar_excess")
            shortage_price_lam = PENALTY_RATE * da_this_hour + rt_this_hour   # 4항 부족비용
            oracle_this_hour_lam = oracle_profit_train_unscaled_by_hour[hour]

            for i in range(n_train_obs):
                profit_i = (
                    da_this_hour[i] * x_lam[i]
                    + rt_this_hour[i] * yplus_lam[i]
                    - shortage_price_lam[i] * yminus_lam[i]
                )
                regret_i = oracle_this_hour_lam[i] - profit_i
                gmodel_lam.addConstr(s_lam[i] >= regret_i - zeta_lam)

            cvar_obj_lam = W1 * lam * (n_train_obs * zeta_lam + s_lam.sum() / (1.0 - CVAR_ALPHA))
            gmodel_lam.setObjective(mean_regret_obj_lam + cvar_obj_lam, GRB.MINIMIZE)
        else:
            gmodel_lam.setObjective(mean_regret_obj_lam, GRB.MINIMIZE)

        gmodel_lam.optimize()
        coefficients_by_hour_lam[hour] = beta_lam.X

    # ---- 테스트 100일 롤링 예측 (기존 절차와 동일) ----
    test_forecast_lam = np.zeros((n_test_days, HOURS_PER_DAY))
    previous_day_actual_lam = train_solar[-1]
    for day_index in range(n_test_days):
        feature_vector_lam = np.concatenate([[1.0], previous_day_actual_lam[::-1]])
        for hour in range(HOURS_PER_DAY):
            raw_pred = np.dot(coefficients_by_hour_lam[hour], feature_vector_lam)
            test_forecast_lam[day_index, hour] = min(max(raw_pred, 0.0), 1.0)
        previous_day_actual_lam = test_solar[day_index]

    actual_flat_lam = test_solar.flatten()
    predicted_flat_lam = test_forecast_lam.flatten()

    rmse_lam = np.sqrt(np.mean((actual_flat_lam - predicted_flat_lam) ** 2))
    nrmse_lam = 100.0 * rmse_lam / actual_flat_lam.mean()

    da_flat_lam = test_da_price.flatten()
    rt_flat_lam = test_rt_price.flatten()
    penalty_flat_lam = PENALTY_RATE * da_flat_lam

    commitment_lam = predicted_flat_lam
    mismatch_lam = actual_flat_lam - commitment_lam
    surplus_flat_lam = np.maximum(mismatch_lam, 0.0)
    shortage_flat_lam = np.maximum(-mismatch_lam, 0.0)
    realized_profit_lam = profit_scale * (
        da_flat_lam * commitment_lam
        + rt_flat_lam * surplus_flat_lam
        - (penalty_flat_lam + rt_flat_lam) * shortage_flat_lam
    )

    # ---- 오라클(3후보) - 기존 절차와 동일 ----
    profit_commit0_lam = profit_scale * (rt_flat_lam * actual_flat_lam)
    profit_commitA_lam = np.where(actual_flat_lam <= 1.0, profit_scale * (da_flat_lam * actual_flat_lam), -np.inf)
    surplus1_lam = np.maximum(actual_flat_lam - 1.0, 0.0)
    shortage1_lam = np.maximum(1.0 - actual_flat_lam, 0.0)
    profit_commit1_lam = profit_scale * (
        da_flat_lam * 1.0 + rt_flat_lam * surplus1_lam - (penalty_flat_lam + rt_flat_lam) * shortage1_lam
    )
    oracle_profit_test_lam = np.maximum(np.maximum(profit_commit0_lam, profit_commitA_lam), profit_commit1_lam)

    gap_lam = 100.0 * (oracle_profit_test_lam.sum() - realized_profit_lam.sum()) / oracle_profit_test_lam.sum()

    regret_test_lam = oracle_profit_test_lam - realized_profit_lam
    total_regret_lam = regret_test_lam.sum()
    n_tail_lam = int(np.ceil((1.0 - CVAR_ALPHA) * len(regret_test_lam)))   # 상위 10% 개수
    cvar90_lam = np.sort(regret_test_lam)[-n_tail_lam:].mean()
    max_regret_lam = regret_test_lam.max()

    lambda_sweep_results.append(dict(
        lam=lam, nrmse=nrmse_lam, gap=gap_lam,
        total_regret=total_regret_lam, cvar90=cvar90_lam, max_regret=max_regret_lam,
    ))
    print(f"lambda={lam:.3f}  nRMSE={nrmse_lam:.4f}%  gap={gap_lam:.4f}%  "
          f"TotalRegret={total_regret_lam:.2f}  CVaR90={cvar90_lam:.2f}  MaxRegret={max_regret_lam:.2f}")
...
lams = [r["lam"] for r in lambda_sweep_results]
nrmse_list = [r["nrmse"] for r in lambda_sweep_results]
gap_list = [r["gap"] for r in lambda_sweep_results]
tr_list = [r["total_regret"] for r in lambda_sweep_results]
cvar90_list = [r["cvar90"] for r in lambda_sweep_results]
maxr_list = [r["max_regret"] for r in lambda_sweep_results]

fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(11, 4.5))

ax1b = ax1.twinx()
ax1.plot(lams, nrmse_list, "o-", color="#F97316", label="nRMSE (%)")
ax1b.plot(lams, gap_list, "s-", color="#3B82F6", label="Optimality Gap (%)")
ax1.set_xlabel("λ")
ax1.set_ylabel("nRMSE (%)", color="#F97316")
ax1b.set_ylabel("Optimality Gap (%)", color="#3B82F6")
ax1.set_title("(a) nRMSE / Gap vs λ")
ax1.set_xticks(lams)

ax2.plot(lams, [v/1000 for v in tr_list], "o-", color="#3B82F6", label="Total regret (×1000)")
ax2.plot(lams, cvar90_list, "s-", color="#EF4444", label="CVaR90")
ax2.plot(lams, [v/10 for v in maxr_list], "^-", color="#10B981", label="Max regret (÷10)")
ax2.set_xlabel("λ")
ax2.set_title("(b) Total regret / CVaR90 / Max regret vs λ")
ax2.legend(fontsize=8)
ax2.set_xticks(lams)

plt.suptitle("Additive CVaR λ 스윕 - Proposed AR·수정4항, z03 블록18 (W1=1, W2=20)")
plt.tight_layout()
plt.savefig("fig_lambda_sweep_ar_4term.png", dpi=150)
print("저장 완료: fig_lambda_sweep_ar_4term.png")
