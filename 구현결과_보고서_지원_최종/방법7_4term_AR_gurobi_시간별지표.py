...
for hour in range(HOURS_PER_DAY):                       # 시간대 0부터 11까지 순서대로 하나씩 처리
    ...
    # 4항(profit_change) 수익식의 부족비용: PC = RT + rate*DA
    # (실시간시장 매입비용 RT + 벌금 rate*DA)
...
#
# 기존 Proposed AR의 전체 regret 가중치는 줄이지 않는다.
#
# 목적함수:
#
# W2 * sum(abs error)
...
#
# 기존 전체 regret 항을 그대로 유지하고
# CVaR 항을 추가적인 패널티로 더한다.
#
# W2 * sum(abs error)
# + W1 * sum(regret)
# + W1 * lambda * N * CVaR_alpha(regret)
# ================================================================
